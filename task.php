...
$page = "tasks";
$project_id = $title = $date = $time = $notes = '';

if (isset($_GET['id'])) {
    list($task_id, $title, $date, $time, $project_id, $notes) = get_task(filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT));
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($project_id) || empty($title) || empty($date) || empty($time)) {
        ...
        if (add_task($project_id, $title, $date, $time, $task_id, $notes)) {
        ...
        <div class="col col-70-md col-60-lg col-center">
            ...
            <form class="form-container form-add" method="post" action="task.php">
                <table>
                    ...
                    <tr>
                        <th><label for="notes">Notes<span class="required">*</span></label></th>
                        <td><textarea name="notes" id="notes"><?php echo htmlspecialchars($notes); ?></textarea></td>
                    </tr>
                </table>
                 <?php
                if (!empty($task_id)) {
