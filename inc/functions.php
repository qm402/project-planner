...
function get_task($task_id){
    ...
    $sql = 'SELECT task_id, title, date, time, project_id, notes FROM tasks WHERE task_id = ?';
...
function add_task($project_id, $title, $date, $time, $task_id=null, $notes){
    include 'connection.php';
    
    if ($task_id) {
        $sql = 'UPDATE tasks SET project_id = ?, title = ?, date = ?, time = ?, notes = ? WHERE task_id = ?';
    } else {
        $sql = 'INSERT INTO tasks(project_id, title, date, time, notes) VALUES(?, ?, ?, ?,?)';
    }
    ...
    try {
        ...
        $results->bindValue(5, $notes, PDO::PARAM_INT);
        if ($task_id) {
            $results->bindValue(6, $task_id, PDO::PARAM_INT);
        }
